...
/************************/\n";
Comment
"Write `a + b mod 2 ^ (64 * len)` in `output`.

  This functions returns the carry.

  The arguments a, b and the outparam output are meant to be `len` limbs in size, i.e. uint64_t[len]"]
val add: len:BN.meta_len t_limbs -> BN.bn_add_eq_len_st t_limbs len

[@@ Comment "Write `a - b mod 2 ^ (64 * len)` in `output`.
...
[@@ Comment "Write `(a + b) mod n` in `output`.
  ...
  The arguments a, b, n and the outparam output are meant to be `len` limbs in size, i.e. uint64_t[len].
...
[@@ Comment "Write `a * b` in `output`.
  ...
  The outparam output is meant to be `2*len` limbs in size, i.e. uint64_t[2*len]."]
...
[@@ Comment "Write `a mod n` in `output`.
  ...
  The argument n and the outparam output are meant to be `len` limbs in size, i.e. uint64_t[len].
...
[@@ Comment "Write `a ^ b mod n` in `output`.
  ...
  The arguments a, n and the outparam output are meant to be `len` limbs in size, i.e. uint64_t[len].
...
[@@ Comment "Deallocate the memory previously allocated by Hacl_Bignum64_mont_ctx_init.
  ...
  The argument ctx is a montgomery context obtained through Hacl_Bignum64_mont_ctx_init."]
val mont_ctx_free: MA.bn_field_free_st t_limbs

[@@ Comment "Write `a mod n` in `output`.

  The argument a is meant to be `2*len` limbs in size, i.e. uint64_t[2*len].
  The outparam output is meant to be `len` limbs in size, i.e. uint64_t[len].
  The argument ctx is a montgomery context obtained through Hacl_Bignum64_mont_ctx_init."]
val mod_precomp: len:Ghost.erased _ -> BS.bn_mod_slow_ctx_st t_limbs len

[@@ Comment "Write `a ^ b mod n` in `output`.

  The arguments a and the outparam output are meant to be `len` limbs in size, i.e. uint64_t[len].
  The argument ctx is a montgomery context obtained through Hacl_Bignum64_mont_ctx_init.
...
[@@ Comment "Write `a ^ (-1) mod n` in `output`.
  ...
  The argument a and the outparam output are meant to be `len` limbs in size, i.e. uint64_t[len].
  The argument ctx is a montgomery context obtained through Hacl_Bignum64_mont_ctx_init.
